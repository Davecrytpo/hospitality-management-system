  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        options: { emailRedirectTo: `${window.location.origin}/admin/login`, data: { full_name: fullName, role: "admin" } },
      });
      if (signUpError) throw signUpError;
      if (data.user) { 
        toast.success("Admin access request sent. You can now login."); 
      ...
      <div className="relative hidden lg:flex flex-col justify-between p-10 overflow-hidden bg-gradient-hero text-white">
        ...
        <div className="relative z-10 space-y-6 max-w-md">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 backdrop-blur px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em]">
            <Sparkles className="h-3 w-3" /> Admin enrollment
          </div>
          <h2 className="font-display text-4xl font-bold leading-tight">Join the administrative team and lead with care.</h2>
          <p className="text-white/80 text-base leading-relaxed">Unified clinical and operational oversight. Sign up to request administrative privileges.</p>

          <div className="grid grid-cols-3 gap-3 pt-4">
            {[
              { icon: ShieldCheck, label: "Full Control" },
              { icon: Activity, label: "Analytics" },
              { icon: HeartPulse, label: "Compliance" },
            ].map((f) => (
      ...
      <div className="flex items-center justify-center p-6 sm:p-10 relative">
        ...
        <Card className="relative z-10 w-full max-w-[460px] border-border/60 shadow-elevated rounded-2xl bg-card">
          <CardHeader className="space-y-3 pb-4">
            ...
            <div>
              <CardTitle className="text-2xl font-display font-bold tracking-tight">Request Admin Access</CardTitle>
              <CardDescription className="text-sm mt-1 flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 text-accent" /> Secure registration for administrators
              </CardDescription>
            </div>
          </CardHeader>
          ...
          <CardContent className="pt-2">
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Input id="signup-name" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Admin Name" required className="h-11 rounded-lg" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="signup-email" className="text-xs font-semibold text-foreground/80">Work Email</Label>
                <Input id="signup-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@ontimemedical.com" required className="h-11 rounded-lg" />
              </div>
              <div className="space-y-1.5">
              ...
              <Button type="submit" className="w-full h-11 font-semibold text-sm rounded-lg bg-gradient-brand hover:opacity-95 transition-opacity" disabled={isLoading}>
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Request Admin Access"}
              </Button>
            </form>

            <div className="mt-6 flex flex-col gap-3">
              <div className="text-center text-xs text-muted-foreground">
                Already have an admin account? <Link to="/admin/login" className="text-primary font-semibold hover:underline">Sign in here</Link>
              </div>
