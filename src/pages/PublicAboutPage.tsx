...
import { findSectionByName, findSectionByType } from "@/features/cms/originalSiteAdapters";
...
export default function PublicAboutPage() {
  ...
  const { data: cmsPage } = useCmsPage("about-us");
  const { data: cmsServices = [] } = useCmsServices();
  const { data: cmsSettings } = useCmsSiteSettings();

  const fallbackPage = cmsDefaults.pages.find((page) => page.slug === "about-us") ?? cmsDefaults.pages[0];
  ...
  const featureSection = findSectionByType(page.sections, "featureGrid");
  const missionSection = findSectionByName(page.sections, /mission/i) ?? findSectionByType(page.sections, "richText");
  const storySection = findSectionByName(page.sections, /story/i) ?? findSectionByType(page.sections, "richText", 1) ?? missionSection;
  const communitySection = findSectionByName(page.sections, /community/i);
  const insuranceSection = findSectionByName(page.sections, /insurance/i) ?? findSectionByType(page.sections, "gallery");

  const phoneLabel = settings.contact.phone || "410-754-4343";
  const emailLabel = settings.contact.email || "info@ontimemedgroup.com";
  const addressLabel = settings.contact.address || "9010 Pulaski Highway, Baltimore, MD 21237";
  const displayedHeroHighlights =
    heroSection?.items.length
      ? heroSection.items.slice(0, 4).map((item, index) => ({
          title: item.title,
          icon: item.icon ? getCmsIcon(item.icon) : heroHighlights[index]?.icon ?? UserRound,
          accent: item.badge?.toLowerCase().includes("red") ? "red" : heroHighlights[index]?.accent ?? "blue",
        }))
      : heroHighlights;
  const displayedFeaturePanels =
    featureSection?.items.length
      ? featureSection.items.slice(0, 4).map((item, index) => ({
          title: item.title,
          description: item.description || "",
          icon: item.icon ? getCmsIcon(item.icon) : featurePanels[index]?.icon ?? CalendarDays,
        }))
      : featurePanels;
  const displayedCommunityBullets =
    communitySection?.items.length
      ? communitySection.items.map((item) => item.title || item.description || "")
      : communityBullets;
  const displayedInsurance =
    insuranceSection?.items.length
      ? insuranceSection.items.map((item, index) => ({
          name: item.title,
          src: item.image?.url ?? "",
          className: insuranceLogos[index]?.className ?? "h-9 sm:h-10",
        }))
      : insuranceLogos;
  ...
  const missionBody = preferPublicCopy(
    missionSection?.body,
    "To deliver timely, accessible, and comprehensive healthcare that empowers individuals and families to live healthier, fuller lives.",
  );
  const storyBody = preferPublicCopy(
    storySection?.body,
    "On Time Medical Group was founded with a simple belief: everyone deserves quality care - on time, every time. We bring together experienced providers and a wide range of services under one roof to meet your needs at every stage of life.",
  );
  const communityTitle = preferPublicCopy(communitySection?.title, "We Care For Our Community");
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(19,48,107,0.07),transparent_24%),radial-gradient(circle_at_top_right,_rgba(239,32,39,0.04),transparent_18%),#ffffff] text-[#13306b]">
      ...
      <main className="overflow-x-hidden bg-transparent">
          ...
          <div className={`${shellClassName} relative`}>
              ...
              <div className="pt-2 lg:pt-7">
                <div className="max-w-[480px]">
                  <h1 className="hero-mock-title text-[3.25rem] font-black uppercase leading-[0.9] tracking-tight text-[#13306b] sm:text-[3.9rem] lg:text-[4.55rem]">
                    {preferPublicCopy(heroSection?.title, "About Us")}
                  </h1>
                  <SectionAccentLine />
                  <p className="mt-6 text-[1rem] leading-8 text-[#13306b] sm:text-[1.02rem]">{aboutBody}</p>
                </div>
              </div>

              <div className="relative z-10 flex justify-center lg:justify-end">
                <div className="relative w-full overflow-hidden rounded-[28px] shadow-[0_26px_60px_-40px_rgba(19,48,107,0.42)]">
                  <img src={heroSection?.image?.url || aboutHeroImage} alt={heroSection?.image?.alt || "Doctor speaking with a patient during an appointment"} className="w-full object-cover object-center" loading="eager" />
            ...
            <div className="relative z-20 mt-7 overflow-hidden rounded-[22px] border border-[#e7edf8] bg-white shadow-[0_24px_56px_-38px_rgba(19,48,107,0.32)] lg:-mt-14 lg:max-w-[636px]">
              <div className="grid grid-cols-2 md:grid-cols-4">
                {displayedHeroHighlights.map((item, index) => (
                  <div
                    key={item.title}
                    className={cn(
                      "flex items-center gap-3 px-4 py-5 sm:px-5",
                      index < displayedHeroHighlights.length - 1 && "border-b border-[#eef3fb] md:border-b-0 md:border-r",
                    )}
                  >
                    <OutlineIcon {...item} />
        ...
        <section className={`${shellClassName} py-6 lg:py-7`}>
          <div className="grid gap-5 lg:items-start lg:grid-cols-[0.96fr_0.88fr_0.88fr_1fr]">
            <article className="self-start overflow-hidden rounded-[22px] border border-[#dfe7f5] bg-white shadow-[0_22px_48px_-38px_rgba(19,48,107,0.3)]">
              <img src={missionSection?.image?.url || storySection?.image?.url || aboutBuildingImage} alt={missionSection?.image?.alt || storySection?.image?.alt || "On Time Medical Group building exterior"} className="h-auto w-full" loading="lazy" />
            </article>
            ...
            <article className="relative overflow-hidden rounded-[22px] border border-[#dfe7f5] bg-white px-6 py-6 shadow-[0_22px_48px_-38px_rgba(19,48,107,0.3)] sm:px-7">
              <div className="flex items-center gap-3 text-[#13306b]">
                ...
                <div>
                  <h2 className="hero-mock-title text-[1.75rem] font-black uppercase leading-none text-[#13306b]">{preferPublicCopy(missionSection?.title, "Our Mission")}</h2>
                  <div className="mt-2 h-[3px] w-10 rounded-full bg-[#ef2027]" />
                </div>
              </div>
              <p className="mt-8 text-[1rem] leading-8 text-[#13306b]">{missionBody}</p>
            ...
            <article className="rounded-[22px] border border-[#dfe7f5] bg-white px-6 py-6 shadow-[0_22px_48px_-38px_rgba(19,48,107,0.3)] sm:px-7">
              <div className="flex items-center gap-3 text-[#13306b]">
                ...
                <div>
                  <h2 className="hero-mock-title text-[1.75rem] font-black uppercase leading-none text-[#13306b]">{preferPublicCopy(storySection?.title, "Our Story")}</h2>
                  <div className="mt-2 h-[3px] w-10 rounded-full bg-[#ef2027]" />
                </div>
              </div>
              <p className="mt-8 text-[1rem] leading-8 text-[#13306b]">{storyBody}</p>
            </article>
            ...
            <article className="rounded-[22px] border border-[#dfe7f5] bg-white px-6 py-6 shadow-[0_22px_48px_-38px_rgba(19,48,107,0.3)] sm:px-7">
              ...
              <ul className="mt-7 space-y-3">
                {displayedCommunityBullets.map((item) => (
              ...
              <div className="mt-9 text-center">
                ...
                <p className="mx-auto mt-2 max-w-[250px] text-[0.92rem] leading-6 text-[#13306b]">
                  {preferPublicCopy(communitySection?.body, "Proudly serving our neighbors and strengthening the communities we call home.")}
                </p>
              </div>
            </article>
          </div>
        ...
        <section className={`${shellClassName} pb-4 pt-3`}>
          ...
          <p className="mt-3 text-center text-sm font-bold text-[#13306b]">{preferPublicCopy(insuranceSection?.body, "...and many more.")}</p>
        </section>
