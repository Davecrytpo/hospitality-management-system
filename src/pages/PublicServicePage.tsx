...
import { useCmsService, useCmsServices, useCmsSiteSettings } from "@/features/cms/hooks";
  ...
  return (
    <div className={cn("grid md:grid-cols-3", !isLast && "border-b border-[#e8eef8]")}>
      {offerings.map((offering, index) => (
        <article key={offering.title} className={cn("px-4 py-8 sm:px-6 lg:px-8", index < offerings.length - 1 && "md:border-r md:border-[#e8eef8]")}>
          <div className="flex items-start gap-4">
            <div className={cn("flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-full", offering.accent === "red" ? "bg-[#fff0f0] text-[#ef2027]" : "bg-[#eff5ff] text-[#13306b]")}>
...
function ConditionCard({ offering }: { offering: ServiceOffering }) {
  return (
    <article className="flex h-full flex-col rounded-[18px] border border-[#e4ebf7] bg-white px-6 py-7 shadow-[0_18px_40px_-38px_rgba(19,48,107,0.18)]">
      <div className={cn("flex h-[72px] w-[72px] items-center justify-center rounded-full", offering.accent === "red" ? "bg-[#fff0f0] text-[#ef2027]" : "bg-[#eff5ff] text-[#13306b]")}>
        <offering.icon className="h-8 w-8" strokeWidth={1.8} />
      </div>
...
export default function PublicServicePage() {
  ...
  const fallbackService = getServicePageContent(slug);
  const { data: cmsService } = useCmsService(slug);
              ...
              <div className="pt-2">
                ...
                <h1 className="hero-mock-title text-[3.15rem] font-black leading-[0.92] tracking-tight text-[#13306b] sm:text-[4rem] lg:text-[4.55rem]">
                  {service.titleLines.map((line) => (
                    <span key={line.text} className={cn("block", line.size === "small" && "text-[2.05rem] leading-[1.02] sm:text-[2.3rem] lg:text-[2.45rem]")}>
                      {line.text}
                    </span>
                  ))}
                </h1>
        ...
        <section className="pt-5 lg:pt-7">
          <div className={shellClassName}>
            <div className="text-center">
              ...
              {service.sectionSubtitle.length > 0 && <p className="mx-auto mt-4 max-w-[760px] text-[1.05rem] leading-8 text-[#13306b]">{service.sectionSubtitle}</p>}
            </div>
            ...
            <div className="rounded-[18px] border border-[#e2e9f6] bg-[#fbfcff] px-6 py-5 shadow-[0_16px_34px_-34px_rgba(19,48,107,0.18)]">
              <div className={cn("grid gap-6", hasStageItems && "lg:grid-cols-[0.48fr_0.52fr] lg:items-center")}>
                <div className={cn("flex items-start gap-5", !hasStageItems && "mx-auto max-w-[760px]")}>
                  <div className={cn("flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-full", service.stageBand.accent === "red" ? "bg-[#fff0f0] text-[#ef2027]" : "bg-[#eff5ff] text-[#13306b]")}>
                ...
                {hasStageItems && (
                  <div className={cn("grid gap-3", service.stageBand.items.length === 3 ? "md:grid-cols-3" : service.stageBand.items.length === 4 ? "md:grid-cols-4" : "md:grid-cols-5")}>
                    {service.stageBand.items.map((item, index) => (
                      <div key={item.label} className={cn("flex flex-col items-center justify-center gap-3 border-t border-[#e3e9f5] px-4 pt-5 text-center md:border-t-0 md:pt-0", index > 0 && "md:border-l md:border-[#e3e9f5]")}>
                        <div className={cn("flex h-[64px] w-[64px] items-center justify-center rounded-full", item.accent === "red" ? "bg-[#fff0f0] text-[#ef2027]" : "bg-[#eff5ff] text-[#13306b]")}>
